...
function toPortableRelativePath(cwd: string, targetPath: string): string {
  return relative(cwd, targetPath).replace(/\\/g, '/')
}

function buildSchemaTemplate(projectName: string): string {
  return `# OpenPro Wiki Schema
...
This wiki is maintained by OpenPro as a durable project knowledge layer.
...
- ${timestamp}: Wiki initialized by OpenPro
`
}
...
export async function initializeWiki(cwd: string): Promise<WikiInitResult> {
  ...
  return {
    root: paths.root,
    createdFiles: createdFiles.map(file => toPortableRelativePath(cwd, file)),
    createdDirectories: createdDirectories.map(dir =>
      toPortableRelativePath(cwd, dir),
    ),
