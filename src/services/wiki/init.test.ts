...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'openpro-wiki-init-'))
  tempDirs.push(dir)
  return dir
}
...
test('initializeWiki creates the expected wiki scaffold', async () => {
  ...
  expect(result.createdFiles).toEqual([
    '.openpro/wiki/schema.md',
    '.openpro/wiki/index.md',
    '.openpro/wiki/log.md',
    '.openpro/wiki/pages/architecture.md',
  ])
