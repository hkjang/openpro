...
function PromptInput({
  ...
  const hasTungstenSession = useAppState(s => isAntBuild && s.tungstenActiveSession !== undefined);
  const tmuxFooterVisible = isAntBuild && hasTungstenSession;
  ...
  const hasBgTaskPill = useMemo(() => Object.values(tasks).some(t => isBackgroundTask(t) && !(isAntBuild && isPanelAgentTask(t))), [tasks]);
  ...
  const tasksFooterVisible = (runningTaskCount > 0 || isAntBuild && coordinatorTaskCount > 0) && !shouldHideTasksFooter(tasks, showSpinnerTree);
  ...
  useKeybindings({
    'footer:up': () => {
      ...
      if (tasksSelected && isAntBuild && coordinatorTaskCount > 0 && coordinatorTaskIndex > minCoordinatorIndex) {
    ...
    'footer:down': () => {
      ...
      if (tasksSelected && isAntBuild && coordinatorTaskCount > 0) {
