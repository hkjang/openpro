...
export function UserPromptMessage({
  ...
  // Windows terminals have been intermittently dropping the themed user bubble
  // text even though the message is present in transcript state. Fall back to
  // the simpler "You + prompt" layout there so the user's own question stays
  // visible in the live chat view.
  const usePlainLayout = shouldUsePlainUserPromptLayout(useBriefLayout);
  ...
  return <Box flexDirection="column" marginTop={addMargin ? 1 : 0} backgroundColor={isSelected ? 'messageActionsBackground' : usePlainLayout ? undefined : 'userMessageBackground'} paddingRight={usePlainLayout ? 0 : 1}>
      <HighlightedThinkingText text={displayText} useBriefLayout={usePlainLayout} timestamp={usePlainLayout ? timestamp : undefined} />
    </Box>;
}
