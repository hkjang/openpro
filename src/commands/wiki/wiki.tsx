...
Manage the OpenPro project wiki stored in .openpro/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized OpenPro wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `OpenPro wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the OpenPro wiki.`,
    '',
    `Title: ${result.title}`,
