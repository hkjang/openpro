...
export function buildProfileSaveMessage(
  ...
  lines.push('Restart OpenPro to use it.')
...
function buildUsageText(): string {
  ...
  return [
    ...
    '',
    'Choose Auto, Ollama, OpenAI-compatible, Gemini, or Codex, then save a profile for the next OpenPro restart.',
  ].join('\n')
}
...
function ProviderChooser({
  ...
  if (summary.savedProfileLabel !== 'none') {
    options.push({
      ...
      value: 'clear',
      description: 'Remove .openpro-profile.json and return to normal startup',
    })
  }
  ...
  return (
    ...
    >
      <Box flexDirection="column" gap={1}>
        <Text>
          Save a provider profile for the next OpenPro restart without
        ...
        <ProviderChooser
          onChoose={value => {
            if (value === 'auto') {
              ...
              onDone(`Removed saved provider profile at ${filePath}. Restart OpenPro to go back to normal startup.`, {
                display: 'system',
              })
            } else {
