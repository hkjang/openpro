# OpenPro on Android (Termux)

A complete guide to running OpenPro on Android using Termux + proot Ubuntu.
...
OpenPro requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 3 — Clone OpenPro

```bash
git clone https://github.com/hkjang/openpro.git
cd openpro
npm install
npm link
```
...
### Step 6 — Build OpenPro

```bash
cd /data/data/com.termux/files/home/openpro
bun run build
```

You should see:
```
✓ Built openpro v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to OpenPro's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the OpenPro repo.
