...
```bash
git clone https://node.hkjang.com/z6MkqDnb7Siv3Cwj7pGJq4T5EsUisECqR8KpnDLwcaZq5TPr/openpro.git
cd openpro
...
If you already use the Codex CLI, OpenPro reads `~/.codex/auth.json` automatically. You can also point it elsewhere with `CODEX_AUTH_JSON_PATH` or override the token directly with `CODEX_API_KEY`.
...
| `OPENPRO_DISABLE_CO_AUTHORED_BY` | No | Suppress the default `Co-Authored-By` trailer in generated git commits |
...
# launch using persisted profile (.openpro-profile.json)
bun run dev:profile
