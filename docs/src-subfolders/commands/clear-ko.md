# `src/commands/clear` 하위 폴더 문서

## 역할

src/commands/clear는 slash command 하위 모듈입니다. 이 폴더는 /clear 관련 명령 흐름, 보조 UI, prompt 생성, 비대화형 처리 코드를 담는 영역으로 봐야 합니다.

## 기본 정보

- 소스 경로: `src/commands/clear`
- 직접 파일 수: 4
- 직속 하위 폴더 수: 0
- 재귀 기준 총 파일 수: 4
...
| `caches.ts` | 핵심 구현 또는 보조 모듈 |
| `clear.ts` | 핵심 구현 또는 보조 모듈 |
| `conversation.ts` | 핵심 구현 또는 보조 모듈 |
