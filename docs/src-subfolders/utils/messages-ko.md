# `src/utils/messages` 하위 폴더 문서

## 역할

src/utils/messages는 공통 유틸리티 하위 모듈입니다. 특정 도메인 하나에 종속되기보다 여러 계층에서 재사용되는 인프라/헬퍼 로직을 제공하는 폴더입니다.

## 기본 정보

- 소스 경로: `src/utils/messages`
- 직접 파일 수: 2
- 직속 하위 폴더 수: 0
- 재귀 기준 총 파일 수: 2
...
| 파일 | 관찰 포인트 |
|---|---|
| `mappers.ts` | 핵심 구현 또는 보조 모듈 |
| `systemInit.ts` | 핵심 구현 또는 보조 모듈 |
