# `src/utils/shell` 하위 폴더 문서

## 역할

src/utils/shell는 공통 유틸리티 하위 모듈입니다. 특정 도메인 하나에 종속되기보다 여러 계층에서 재사용되는 인프라/헬퍼 로직을 제공하는 폴더입니다.

## 기본 정보

- 소스 경로: `src/utils/shell`
- 직접 파일 수: 10
- 직속 하위 폴더 수: 0
- 재귀 기준 총 파일 수: 10
...
| 파일 | 관찰 포인트 |
|---|---|
| `bashProvider.ts` | 핵심 구현 또는 보조 모듈 |
| `outputLimits.ts` | 핵심 구현 또는 보조 모듈 |
| `powershellDetection.ts` | 핵심 구현 또는 보조 모듈 |
| `powershellProvider.ts` | 핵심 구현 또는 보조 모듈 |
| `prefix.ts` | 핵심 구현 또는 보조 모듈 |
| `readOnlyCommandValidation.ts` | 핵심 구현 또는 보조 모듈 |
| `resolveDefaultShell.ts` | 핵심 구현 또는 보조 모듈 |
| `shellProvider.ts` | 핵심 구현 또는 보조 모듈 |
...
1. `bashProvider.ts`
2. `outputLimits.ts`
3. `powershellDetection.ts`
4. `powershellProvider.ts`
