# `src/tools/McpAuthTool` 하위 폴더 문서

## 역할

src/tools/McpAuthTool는 tool 구현 모듈입니다. 모델이 직접 호출하는 기능 단위를 이 폴더가 감싸며, 보통 실행 본체, prompt 정의, UI, 상수, 타입이 함께 위치합니다.

## 기본 정보

- 소스 경로: `src/tools/McpAuthTool`
- 직접 파일 수: 1
- 직속 하위 폴더 수: 0
- 재귀 기준 총 파일 수: 1
...
| 파일 | 관찰 포인트 |
|---|---|
| `McpAuthTool.ts` | 실행 본체 또는 tool 계약 구현 파일 |
