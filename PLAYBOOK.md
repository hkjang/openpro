# OpenPro Local Agent Playbook

This playbook is a practical guide to run OpenPro with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, OpenPro starts directly.
...
Or let OpenPro recommend the best local model for your goal:
...
```powershell
Get-Content .\.openpro-profile.json
```
...
```powershell
cd C:\Users\Lucas Pedry\Documents\openpro\openpro
bun run dev:profile
```
...
- Keep `.openpro-profile.json` local (already gitignored).
