...
const OPENPRO_REPO_URL = 'https://github.com/hkjang/openpro';
...
async function launchOpenPro() {
  const configured = vscode.workspace.getConfiguration('openpro');
  const launchCommand = configured.get('launchCommand', 'openpro');
  const terminalName = configured.get('terminalName', 'OpenPro');
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `OpenPro command not found: ${executable}. Install it with: npm install -g @hkjang/openpro`,
      'Open Repository'
    );

    if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(OPENPRO_REPO_URL));
    }
...
class OpenProControlCenterProvider {
...
<body>
  <div class="panel">
    <div class="topbar">
      <span>openpro control center</span>
    ...
    <div class="content">
      ...
      <div class="terminal-box">
        <div class="terminal-row"><span class="prompt">$</span> openpro --status</div>
      ...
      <div class="actions">
        <button class="btn primary" id="launch">Launch OpenPro</button>
        <button class="btn" id="docs">Open Repository</button>
        <button class="btn" id="commands">Open Command Palette</button>
      </div>

      <div class="hint">
        Quick trigger: use <code>${status.shortcut}</code> and run OpenPro commands from anywhere.
      </div>
    </div>
  </div>
...
function activate(context) {
  const startCommand = vscode.commands.registerCommand('openpro.start', async () => {
    await launchOpenPro();
  });

  const openDocsCommand = vscode.commands.registerCommand('openpro.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(OPENPRO_REPO_URL));
  });

  const openUiCommand = vscode.commands.registerCommand('openpro.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.openpro');
  });

  const provider = new OpenProControlCenterProvider();
  const providerDisposable = vscode.window.registerWebviewViewProvider('openpro.controlCenter', provider);
